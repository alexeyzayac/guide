...
# Worker-ноды (создаются две, без использования переменной)
resource "yandex_compute_instance" "vm-node-worker" {
  count = 2
  name        = "vm-node-worker-${count.index + 1}"
  hostname    = "vm-node-worker-${count.index + 1}"
  platform_id = "standard-v3"
  zone        = "ru-central1-a"
